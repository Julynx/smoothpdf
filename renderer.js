...
import { getUIElements, updateWindowTitle, showMessage, hideMessage, updateControlsUI, setupPageObserver } from './js/ui.js';
import { loadPdfDocument, jumpToPage, renderDocumentToLayer } from './js/pdf.js';

const ui = getUIElements();

async function performCrossfadeUpdate(filePath, anchorPage = null, isInstant = false, forceReload = false) {
    await updateState({ isRendering: true });
    try {
        let doc;
        if (!forceReload && filePath === state.currentPdfPath && state.currentPdfDocument) {
        ...
        if (!isInstant) {
            await new Promise((resolve) => {
                state.currentFront.addEventListener('transitionend', resolve, { once: true });
            });
        }
...
async function checkPending() {
    if (state.pendingRenderOptions) {
        ...
        performCrossfadeUpdate(state.currentPdfPath, options.anchorPage, options.isInstant, options.forceReload);
    }
}
...
const containerObserver = new ResizeObserver(() => {
    ...
    resizeTimeout = setTimeout(async () => {
        if (!state.isRendering) {
            await performCrossfadeUpdate(state.currentPdfPath, state.currentPageNumber, true);
        } else {
            await updateState({ pendingRenderOptions: { anchorPage: state.currentPageNumber, isInstant: true } });
        }
    }, 150);
});
...
window.addEventListener('keydown', async (e) => {
    ...
    if ((e.ctrlKey || e.metaKey) && (e.key === '=' || e.key === '+' || e.key === '-')) {
        ...
        if (state.currentZoomMode === 'fit-width' || state.currentZoomMode === 'fit-height') {
            newZoomMode = '1';
        } else {
        ...
        if (newZoomMode !== state.currentZoomMode) {
            ...
            if (state.isRendering) {
                ...
                performCrossfadeUpdate(state.currentPdfPath, state.currentPageNumber, true);
            }
        }
    }
});
...
async function init() {
    try {
        ...
        window.api.onFileUpdated(async (updatedPath) => {
            ...
            if (state.isRendering) {
                await updateState({ pendingRenderOptions: { anchorPage: null, isInstant: false, forceReload: true } });
            } else {
